...
for sample_idx in tqdm(range(len(train_files))):
    ...
    # Output maps
    s_maps = np.zeros((num_slices, num_coils, img_h, img_w), dtype=np.complex64)
    ...
    for idx in tqdm(range(num_slices)):
        ...
        acs_idx = np.arange(num_slices // 2 - acs_lines // 2, num_slices // 2 + acs_lines // 2)
        ...
        # Estimate maps
        s_maps[idx] = (
            bart(1, "ecalib -m1 -W -c0", k_down.transpose((1, 2, 0))[None, ...])
            .transpose((3, 1, 2, 0))
            .squeeze()
        )
